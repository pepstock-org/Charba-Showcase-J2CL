...
public class MatrixClickEventCase extends BaseComposite {
	...
	private final MatrixDataset dataset1;

	public MatrixClickEventCase() {
		// ----------------------------------------------
		// Main element
		// ----------------------------------------------
		...
		chart.addHandler(new DatasetSelectionEventHandler() {
			...
			public void onSelect(DatasetSelectionEvent event) {
				MatrixDataPoint point = dataset1.getDataPoints().get(event.getItem().getIndex());
				Date date = point.getAttributeAsDate(DATE);
				StringBuilder sb = new StringBuilder();
				sb.append("Dataset index: <b>").append(event.getItem().getDatasetIndex()).append("</b><br>");
				sb.append("Full Date: <b>").append(adapter.format(date, "EEEE, dd MMMM yyyy")).append("</b><br>");
				sb.append("Value: <b>").append(point.getValue()).append("</b><br>");
		...
		dataset1.setBackgroundColor(new ColorCallback<DatasetContext>() {
			...
			public Object invoke(DatasetContext context) {
				...
				return HtmlColor.RED.alpha(alpha);
			}
		...
		dataset1.setBorderColor(new ColorCallback<DatasetContext>() {
			...
			public Object invoke(DatasetContext context) {
				...
				MatrixDataPoint point = points.get(context.getDataIndex());
				double alpha = Math.min(Math.max((10 + point.getValue()) / 110, 0), 1);
				return HtmlColor.DARK_RED.alpha(alpha);
			}

		});
		dataset1.setBorderWidth(2);
		dataset1.setHoverBackgroundColor(HtmlColor.YELLOW);
		dataset1.setHoverBorderColor(HtmlColor.ORANGE);
